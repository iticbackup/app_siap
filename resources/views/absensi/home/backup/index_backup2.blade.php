                        ...
                        <table class="table table-striped table-bordered dataTable mb-0">
                            ...
                            <tbody>
                                ...
                                @php
                                    ...
                                    $absens = $fin_pro->where('scan_date', 'LIKE', '%'.$date_live.'%')
                                    ...
                                    if (empty($absens[0])) {
                                        ...
                                        if (empty($absen_masuk)) {
                                            $date_jam_masuk = \Carbon\Carbon::create($absens[0]['scan_date'])->format('H:i');
                                            $jam_masuk = '<a type="button" onclick="detail_absensi_jam_masuk(`' . $absens[0]['scan_date'] . '`,`' . $absens[0]['pin'] . '`,`' . $absens[0]['inoutmode'] . '`)" style="color: blue">' . $date_jam_masuk . '</a>';
                                        } else {
                                            $date_jam_masuk = \Carbon\Carbon::create($absens[0]['scan_date'])->format('H:i');
                                            $jam_masuk = '<a type="button" onclick="detail_absensi_jam_masuk(`' . $absens[0]['scan_date'] . '`,`' . $absens[0]['pin'] . '`,`' . $absens[0]['inoutmode'] . '`)" style="color: blue">' . $date_jam_masuk . ' (' . $absen_masuk->presensi_status->status_info . ')</a>';
                                        }
                                    }

                                    if (empty($absens[1])) {
                                        $presensi_info_pulang = $presensi_info->where('scan_date', 'LIKE', '%' . $date_live . '%')
                                                                                ->where('pin', $biodata_karyawan->pin)
                                        ...
                                        $absen_pulang = $presensi_info->with('presensi_status')
                                                                        ->where('scan_date', 'LIKE', '%' . $date_live . '%')
                                                                        ->where('pin', $biodata_karyawan->pin)
                                                                        ->whereTime('scan_date','>=','12:00:00')
                                        ...
                                        if (empty($absen_keluar)) {
                                            $date_jam_keluar = \Carbon\Carbon::create($absens[1]['scan_date'])->format('H:i');
                                            $jam_keluar = '<a type="button" onclick="detail_absensi_jam_keluar(`' . $absens[1]['scan_date'] . '`,`' . $absens[1]['pin'] . '`,`' . $absens[1]['inoutmode'] . '`)" style="color: blue">' . $date_jam_keluar . '</a>';
                                        } else {
                                            $date_jam_keluar = \Carbon\Carbon::create($absens[1]['scan_date'])->format('H:i');
                                            $jam_keluar = '<a type="button" onclick="detail_absensi_jam_keluar(`' . $absens[1]['scan_date'] . '`,`' . $absens[1]['pin'] . '`,`' . $absens[1]['inoutmode'] . '`)" style="color: red">' . $date_jam_keluar . ' (' . $absen_keluar->presensi_status->status_info . ')</a>';
                                        }
                                    }
                                @endphp
                                <tr>
