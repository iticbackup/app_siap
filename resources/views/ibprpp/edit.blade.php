...
@section('title')
    IBPRPP Periode {{ $ibprpp->ibprpp_periode->periode . ' - ' . $ibprpp->ibprpp_departemen->departemen }}
@endsection
                    ...
                    <div class="card-body">
                        <table class="table table-bordered dt-responsive nowrap"
                            ...
                            <tr>
                                <th>Periode</th>
                                <th>:</th>
                                <th>{{ $ibprpp->ibprpp_periode->periode }}</th>
                            </tr>
                            <tr>
                                <th>Departemen</th>
                                <th>:</th>
                                <th>{{ $ibprpp->ibprpp_departemen->departemen }}</th>
                            </tr>
                            <tr>
                                <th>Kategori Area</th>
                                <th>:</th>
                                <th>
                                    <select name="ibprpp_category_area_id" class="form-control" id="">
                                        ...
                                        @foreach ($category_areas as $category_area)
                                            <option value="{{ $category_area->id }}" {{ $ibprpp->ibprpp_category_area_id == $category_area->id ? 'selected' : null}}>{{ $category_area->category_area }}
                                            </option>
                                        @endforeach
                                    </select>
                                </th>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" name="aktifitas_pekerja" class="form-control"
                                            placeholder="Aktifitas Pekerja" value="{{ $ibprpp->aktivitas_pekerja }}">
                                    </div>
                                    <div class="col-md-6">
                                        <label for="" class="mb-2">Jenis Aktivitas</label>
                                        <select name="jenis_aktivitas" class="form-control">
                                            <option value="">-- Pilih Jenis Aktivitas --</option>
                                            <option value="Rutin" {{ $ibprpp->jenis_aktivitas == 'Rutin' ? 'selected' : null }}>Rutin</option>
                                            <option value="Non Rutin" {{ $ibprpp->jenis_aktivitas == 'Non Rutin' ? 'selected' : null }}>Non Rutin</option>
                                            <option value="Darurat" {{ $ibprpp->jenis_aktivitas == 'Darurat' ? 'selected' : null }}>Darurat</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="repeater-body">
                            <div data-repeater-list="body">
                                @foreach (json_decode($ibprpp->body) as $body)
                                {{-- @dd($body) --}}
                                    ...
                                    <div class="card">
                                        ...
                                        <div class="card-body">
                                            <table class="table table-bordered dt-responsive nowrap"
                                                ...
                                                <tr>
                                                    ...
                                                    <td><input type="text" name="potensi_bahaya" class="form-control"
                                                            placeholder="Potensi Bahaya" value="{{ $body->potensi_bahaya }}"></td>
                                                </tr>
                                            </table>
                                            <div class="repeater-risiko-bahaya">
                                                <div class="card">
                                                    <div class="card-header bg-gradientNew">
                                                            ...
                                                            {{-- <div data-repeater-item=""> --}}
                                                                <table data-repeater-list="risiko_bahaya" class="table table-bordered dt-responsive nowrap"
                                                                    ...
                                                                    <tbody>
                                                                        ...
                                                                        @foreach ($body->risiko_bahaya as $risiko_bahaya)
                                                                        <tr data-repeater-item="">
                                                                            <td>
                                                                                <input type="text" name="risiko_bahaya"
                                                                                    class="form-control" placeholder="Risiko Bahaya" value="{{ $risiko_bahaya->risiko_bahaya }}">
                                                                            </td>
                                                                            <td>
                                                                                <input type="number" name="jumlah_kejadian"
                                                                                    class="form-control text-center"
                                                                                    placeholder="Jumlah Kejadian" value="{{ $risiko_bahaya->jumlah_kejadian }}">
                                                                            </td>
                                                                            <td>
                                                                                <input type="number" name="frekuensi"
                                                                                    class="form-control text-center"
                                                                                    placeholder="Frekuensi (Probability)" value="{{ $risiko_bahaya->frekuensi }}">
                                                                            </td>
                                                                            <td>
                                                                                <input type="number" name="keparahan"
                                                                                    class="form-control text-center"
                                                                                    placeholder="Keparahan (Severity)" value="{{ $risiko_bahaya->keparahan }}">
                                                                            </td>
                                                                            <td>
                                                                                <input type="text" name="penetapan_pengendalian"
                                                                                    class="form-control text-center"
                                                                                    placeholder="Penetapan Pengendalian" value="{{ $risiko_bahaya->penetapan_pengendalian }}">
                                                                            </td>
                                            ...
                                            <div class="repeater-upaya-pengendalian">
                                                <div class="card">
                                                    ...
                                                    <div data-repeater-list="upaya_pengendalian">
                                                        @foreach ($body->upaya_pengendalian as $upaya_pengendalian)
                                                        <div data-repeater-item="">
                                                            <div class="card-body">
                                                                <div class="row">
                                                                    <div class="col-md-5">
                                                                        ...
                                                                        <textarea name="upaya_pengendalian" class="form-control" cols="30" rows="2"
                                                                            placeholder="Upaya Pengendalian">{{ $upaya_pengendalian->upaya_pengendalian }}</textarea>
                                                                    </div>
                                                                    <div class="col-md-5">
                                                                        ...
                                                                        <input type="text" name="penilaian_pengendalian"
                                                                            class="form-control" placeholder="Penilaian Pengendalian" value="{{ $upaya_pengendalian->penilaian_pengendalian }}">
                                                                    </div>
                                            ...
                                            <div class="card">
                                                ...
                                                <div class="card-body">
                                                    <textarea name="pic_wewenang" class="form-control editor" cols="30" rows="5" placeholder="PIC / Wewenang">{{ $body->pic_wewenang }}</textarea>
                                                </div>
                                            </div>
                                            <div class="card">
                                                <div class="card-header bg-gradientNew">
                                                ...
                                                <div class="card-body">
                                                    <textarea name="regulasi_terkait" class="form-control editor" cols="30" rows="5"
                                                        placeholder="Regulasi Terkait">{!! $body->regulasi_terkait !!}</textarea>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
@section('script')
    ...
    <script>
        ...
        $('#form-update').submit(function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('qhse.ibprpp.departemen_update', ['periode' => $ibprpp->ibprpp_periode->periode, 'departemen_id' => $ibprpp->ibprpp_departemen->id, 'id' => $ibprpp->id]) }}",
                data: formData,
                contentType: false,
                processData: false,
                beforeSend: function() {
                ...
                success: (result) => {
                    if (result.success != false) {
                        ...
                        setTimeout(() => {
                            window.location.href="{{ route('qhse.ibprpp.departemen_preview',['periode' => $ibprpp->ibprpp_periode->periode, 'departemen_id' => $departemen_id]) }}"
                        }, 3000);
                    } else {
